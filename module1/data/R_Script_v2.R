...
sim_folder <- "C:\\Users\\mooret\\Dropbox\\Virginia\\ProjectEDDIE\\Modules1-4\\1Climate Change Effects on Lake Temperatures\\Data" ##!! Edit this line 
...
get_nml_value(nml, 'horse') ##!! Use this command to find out the values of 
...
plot_temp(baseline) # This plots your simulated water 
...
plot_var_compare(nc_file = baseline, field_file, var_name = "temp") # Plot your GLM model (simulated) data vs. 
...
MyExpRootDir <- file.path(sim_folder, 'MyExpRoot') # Met files and nml are here		
MyResultsDir <- file.path(sim_folder, 'MyResults') # Outputs will go here		
...
grep(MyExpRootDir, MyResultsDir)
